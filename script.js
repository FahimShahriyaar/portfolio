...
function hash(obj) {
    ...
    for (let j = 0; j < obj.length; j++){
        const x= tmp[j].published?['Github','','']:[tmp[j].info,'disabled','opacity-75'];
        const dv = document.createElement('div');
        dv.setAttribute('class',`col-sm-4 col-10 ${x[2]}`)
        dv.innerHTML=`<div><img src="assets/${tmp[j].thumbnail}" alt="${tmp[j].title}">
                    ...
                    <a href="${tmp[j].github_link}" target="_blank" type="button" class="btn btn-outline-dark ${x[1]}">${x[0]}</a></div>`;
